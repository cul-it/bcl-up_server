...
module PrependedLinkedData::SearchQuery
  ...
  def search(query, request_header: {}, language: nil, replacements: {}, subauth: nil, context: false, performance_data: false) # rubocop:disable Metrics/ParameterLists
    ...
    request_header = setup_search(request_header:, language:, replacements:, subauth:,
                                  context:, performance_data:)
    @phid = BclUpServer.config.performance_cache.new_entry(authority: authority_name, action: 'search')
    begin
      full_results = super
      update_performance_history_record(full_results)
    rescue Exception => e # rubocop:disable Lint/RescueException
      BclUpServer.config.performance_cache.destroy(@phid)
      raise e
    end
...
private
  ...
  def update_performance_history_record(full_results)
    return BclUpServer.config.performance_cache.destroy(@phid) unless full_results.is_a?(Hash) && full_results.key?(:performance)
    updates = { action_time_ms: (BclUpServer::TimeService.current_time_s - @start_time_s) * 1000,
