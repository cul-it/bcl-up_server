...
module BclUpServer
  class TimeService
    class << self
      ...
      def current_time
        Time.now.in_time_zone(BclUpServer.config.preferred_time_zone_name)
      end
      ...
      def pretty_time(dt)
        dt.in_time_zone(BclUpServer.config.preferred_time_zone_name).strftime("%m/%d/%Y - %I:%M %p")
      end

      # @param dt [ActiveSupport::TimeWithZone] date time stamp
      # @return [String] string version of date formatted with just date (e.g. "02/01/2020")
      def pretty_date(dt)
        dt.in_time_zone(BclUpServer.config.preferred_time_zone_name).strftime("%m/%d/%Y")
      end

      # @param dt [ActiveSupport::TimeWithZone] date time stamp
      # @return [String] string version of date formatted with just date (e.g. "2020-02-01")
      def pretty_query_date(dt)
        dt.in_time_zone(BclUpServer.config.preferred_time_zone_name).strftime("%Y-%m-%d")
      end
    end
  end
end
