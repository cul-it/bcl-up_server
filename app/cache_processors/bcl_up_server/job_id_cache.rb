...
module BclUpServer
  class JobIdCache
    class << self
      ...
      # @param job_key [String] key unique to the job being run (e.g. "BclUpServer::Jobs::MonitorTestsJob")
      ...
      def active_job_id?(job_key:, job_id:, expires_in: 30.minutes)
        cached_job_id = Rails.cache.fetch(cache_key(job_key), expires_in:, race_condition_ttl: 5.minutes) { job_id }
