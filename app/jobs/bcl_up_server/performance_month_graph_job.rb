  ...
  private
    ...
    def generate_graphs_for_authority(authority_name:)
      [SEARCH, FETCH, ALL_ACTIONS].each_with_object({}) do |action, hash|
        hash[action] = generate_30_day_graph(authority_name:, action:)
      end
    end

    def generate_30_day_graph(authority_name:, action:)
      data = graph_data_service.calculate_last_30_days(authority_name:, action:)
      graphing_service.generate_month_graph(authority_name:, action:, data:)
    end
